...
def hamta_sokdata(sokord):
    ...
    data = [{"keywords": [sokord], "location_code": 2170, "language_code": "es"}]
...
def skapa_checkout(email):
    ...
    session = stripe.checkout.Session.create(
        ...
        success_url="https://seo-argentina-ib26kguspq75vu9imb499y.streamlit.app?paid=true",
        cancel_url="https://seo-argentina-ib26kguspq75vu9imb499y.streamlit.app",
    )
    return session.url

def ar_prenumerant(email):
    res = supabase.table("subscribers_colombia").select("email").eq("email", email).execute()
    return len(res.data) > 0

def spara_prenumerant(email):
    if not ar_prenumerant(email):
        supabase.table("subscribers_colombia").insert({"email": email}).execute()

st.title("SEO Colombia")
...
if st.session_state.user is None:
    ...
    if params.get("paid") == "true":
        ...
        st.success("¡Pago confirmado! ¡Bienvenido a SEO Colombia Pro!")
    ...
    if prenumerant:
        ...
        sokord = st.text_input("Introduce una palabra clave:", placeholder="ej: agencia de marketing Bogota")
        if st.button("Buscar") and sokord:
            with st.spinner("Buscando datos en Google Colombia..."):
                ...
                try:
                    ...
                    st.success(f"Datos de Google Colombia para: {sokord}")
                except:
        ...
        if st.button("Suscribirse a SEO Colombia Pro - $19 USD/mes"):
            url = skapa_checkout(st.session_state.user.email)
            st.markdown(f"[Haz clic aquí para pagar]({url})")

st.divider()
st.caption("SEO Colombia  - Hecho para el mercado colombiano")